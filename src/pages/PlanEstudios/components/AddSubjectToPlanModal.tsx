...
const AddSubjectToPlanModal: React.FC<Props> = ({
  ...
  const validate = () => {
    ...
    if (!semester || semester < 1 || semester > 12) newErrors.semester = 'El semestre debe estar entre 1 y 12'
    setErrors(newErrors)
    return Object.keys(newErrors).length === 0
  }

  const handleConfirm = () => {
    if (!validate() || !selectedSubject) return
    onConfirm({
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-40 px-4">
      <div className="flex w-full max-w-lg flex-col rounded-2xl bg-white shadow-xl dark:bg-boxdark">
        ...
        <div className="flex items-center justify-between border-b border-stroke px-6 py-4 dark:border-strokedark">
          <h2 className="text-base font-semibold text-black dark:text-white">Agregar asignatura al plan</h2>
          <button type="button" onClick={handleClose} className="text-body hover:text-black dark:hover:text-white">
        ...
        <div className="space-y-4 px-6 py-5">
          ...
          <div>
            ...
            <div className={`max-h-44 overflow-y-auto rounded-lg border ${errors.subject ? 'border-danger' : 'border-stroke dark:border-strokedark'}`}>
              {availableSubjects.length === 0 ? (
                <p className="px-3 py-3 text-sm text-body dark:text-bodydark">
                  {searchQuery ? 'Sin resultados para la búsqueda' : 'Todas las asignaturas ya están en el plan'}
                </p>
              ) : (
                availableSubjects.map((subject) => (
                  <button
                    key={subject.id}
                    type="button"
                    onClick={() => setSelectedSubjectId(Number(subject.id))}
                    ...
                    } ${!subject.is_active ? 'opacity-40 cursor-not-allowed' : ''}`}
                    disabled={!subject.is_active}
                  >
                    <span>
                      <span className="font-mono text-xs text-body dark:text-bodydark mr-2">{subject.code}</span>
                      {subject.name}
                      {!subject.is_active && <span className="ml-2 text-xs text-danger">(inactiva)</span>}
                    </span>
                    <span className="text-xs text-body dark:text-bodydark">{subject.credits} cr.</span>
                  </button>
                ))
              )}
            </div>
          ...
          <div>
            ...
            />
            {errors.semester && <p className="mt-1 text-xs text-danger">{errors.semester}</p>}
          </div>
        </div>

...
const ListCareer = () => {
  ...
  const [archiveActiveSemesters, setArchiveActiveSemesters] = useState<{ name: string; period: string }[]>([])
  ...
  const handleSubmit = async () => {
    try {
      if (selectedCareer) {
        await careerBusiness.updateCareer(selectedCareer.id, formData as UpdateCareerDto)
  ...
  const handleView = async (career: Career) => {
    ...
    try {
      ...
      const associatedSemesters = semesters.filter((semester) => semester.career_id === career.id)

      setDetailSemesters(associatedSemesters)

      setDetailStudentCount(
        registrations.filter(
          (registration) => registration.career_id === career.id && registration.is_active
        ).length
      )

      setDetailStudyPlanCount(
        studyPlans.filter((studyPlan) => studyPlan.career_id === career.id).length
      )
    } catch (error) {
      console.error(error)
    }
    ...
    <div>
      <div className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-500">Gestiona las carreras académicas del sistema.</p>
        </div>
